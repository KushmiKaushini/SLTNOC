...
class commonIssuesPage extends StatefulWidget {
  final String title;

  const commonIssuesPage({
    Key? key,
    required this.title,
  }) : super(key: key);

  @override
  _commonIssuesPageState createState() => _commonIssuesPageState();
}

class _commonIssuesPageState extends State<commonIssuesPage> {
  ...
  Future<void> fetchData() async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final manualIssues = await _fetchManualIssues();
        data.insertAll(0, manualIssues);
  ...
  Future<List<Map<String, dynamic>>> _fetchManualIssues() async {
    try {
      final manualItems = await _manualEscalationService.fetchActive();
      return manualItems
          .where((item) => item.escalationType == 'COMMON ISSUES')
          .map((item) {
        return {
              ...
              rows: commonIssues.map((index) {
                return DataRow(
                  cells: [
                    ...
                    //   Container(
                    //     alignment: Alignment.centerLeft, // Align text left
                    //     height: 100, // Increase the height to accommodate the description
                    //     child: RichText(
                    //       text: TextSpan(
                    //         style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w500, color: Color(0xFF00305e)),
                    //         children: [
                    //           TextSpan(text: workGroup['Docket'], style: const TextStyle(fontSize: 16, fontWeight: FontWeight.bold)),
                    //           const TextSpan(text: '\n'), // New line
                    //           WidgetSpan(
                    //             child: FittedBox(
                    ...
                    //               child: Text(
                    //                 workGroup['Status'],
                    //                 style: const TextStyle(fontSize: 14, color: Color(0xFF0056A2), fontWeight: FontWeight.w500, height: 1.5),
                    //               ),
                    //             ),
                    //           ), // Wrap the second line with WidgetSpan containing FittedBox
                    //           const TextSpan(text: '\n'), // New line
                    //           TextSpan(text: workGroup['Description'], style: const TextStyle(fontSize: 14, color: Colors.green, height: 2.0)), // Adjust height as needed
                    //         ],
                    //       ),
                    //     ),
                    //   ),
